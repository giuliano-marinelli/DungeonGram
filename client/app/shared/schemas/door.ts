...
export class Door extends Schema {
  ...
  doMesh() {
    ...
    if (this.size == 'small') height = height / 4;
    ...
    var material = new BABYLON.StandardMaterial("door", this.parameters.scene);
    ...
    this.mesh.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnPointerOverTrigger, () => {
      if (this.parameters.controller.activeTool?.name == 'doors')
    ...
    this.mesh.actionManager.registerAction(new BABYLON.ExecuteCodeAction(BABYLON.ActionManager.OnLeftPickTrigger, (e) => {
      if (this.parameters.controller.activeTool?.name == 'doors'
        ...
        this.parameters.controller.send('game', 'door', { id: this.id, action: 'remove' });
    }));

    //cast shadows with character light (if it's not an only collider door)
    ...
    this.parameters.world.updateDoorVisibility(this);
  }
